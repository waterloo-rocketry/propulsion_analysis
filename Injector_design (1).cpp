...
//****Parameters*********
	...
	double P2=100;//psi, absolute post-injector pressure
	double P2F=700;//max downstream pressure
	double P2int=10;//step intervals


	double Cd=1;//
	double Cvc=1;//0.64;//vena contracta effect 0.64 for small L/D, 1 for large L/D
	double k=0.5;//local loss coeff
	double Ks = 0.03;//roughness value (mm);
	double Aratio=0;//A1/A2

	double L=25.4;//orifice length (mm)
	double Di=1.5;//orifice diameter (mm)
	double N=1;//number of orifices
